...
_confPath=`echo ${_confPathSpec} | awk -F= '{print $2}'`
if [[ "${_confPath}" = "" ]]
then
...
_timeout=`echo ${_timeoutSpec} | awk -F= '{print $2}'`
if [[ "${_timeout}" = "" ]]
then
...
#--------------------------------------------------------------------------------
_verbose_msg "Azure Linux agent: verify existence of pre-script within directory \"/var/lib/waagent\""
...
_verbose_msg "Azure Linux agent: verify existence of post-script within directory \"/var/lib/waagent\""
...
do
	...
	#------------------------------------------------------------------------
	_verbose_msg "DB instance \"${_oraSid}\": verify SYSBACKUP group in \"${_oraHome}/rdbms/lib/config.c\""
